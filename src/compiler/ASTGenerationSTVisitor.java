...
public class ASTGenerationSTVisitor extends FOOLBaseVisitor<Node> {
	...
	public Node visitCldec(CldecContext ctx) {
		...
		final String superId = ctx.EXTENDS() == null ? null : ctx.ID(1).getText();


		final List<FieldNode> fieldList = new ArrayList<>();
		int j = superId != null ? 2 : 1;
		for(int i = j ; i < ctx.ID().size(); i++) {
			final String fieldId = ctx.ID(i).getText();
			final TypeNode fieldType = (TypeNode) visit(ctx.type(i-j));
